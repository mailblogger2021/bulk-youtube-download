...
for /l %%i in (1,1,%count%) do (
    ...
    set "current_url=!current_url:,=!"
    yt-dlp.exe --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36" --proxy socks5://178.128.113.118:23128 --ffmpeg-location "ffmpeg.exe" -f "bestvideo[ext=mp4][height<=1080]+bestaudio[ext=m4a]/best[ext=mp4]" --embed-subs --embed-thumbnail --embed-metadata --no-playlist --xff SG -o "tiktok\%%(title)s [%%(id)s].%%(ext)s" !current_url!
