...
for /l %%i in (1,1,%count%) do (
	...
	set "current_url=!current_url:,=!"
	yt-dlp.exe --ffmpeg-location "ffmpeg.exe" -f "bestvideo[ext=mp4][height<=1080]+bestaudio[ext=m4a]/best[ext=mp4]" --embed-subs --embed-thumbnail --embed-metadata --no-playlist -o "tiktok\%%(title)s [%%(id)s].%%(ext)s" !current_url!
    echo.
)
